...
- **Recommended**: Blender 5.0
- **Backward Compatibility**: The add-on is technically compatible with Blender 3.2+ (due to Python API usage), though the author primarily supports and recommends version 5.0.
...
Możesz dostosować maksymalną liczbę wyświetlanych wyników w preferencjach dodatku:

1. Wejdź w **Edycja > Preferencje > Dodatki** (Edit > Preferences > Add-ons).
2. Znajdź **Sidebar Tab Search**.
3. Rozwiń szczegóły i dostosuj opcję **Max Search Results** (Domyślnie: 25, Maks: 500).

### Kompatybilność

- **Zalecana**: Blender 5.0
- **Kompatybilność wsteczna**: Dodatek jest technicznie kompatybilny z Blenderem 3.2+ (ze względu na użyte API Pythona), chociaż autor głównie wspiera i zaleca wersję 5.0.
